...
# Simple script to run DARIA with a patch for Python 3.13 compatibility
...
echo -e "${GREEN}===========================================${NC}"
echo -e "${GREEN}  DARIA with Python 3.13 Patch ${NC}"
echo -e "${GREEN}===========================================${NC}"
...
lsof -i :5025 | awk 'NR>1 {print $2}' | xargs kill -9 2>/dev/null || true
...
cd "${SCRIPT_DIR}/audio_tools" && python stt_service.py --port 5016 > "${SCRIPT_DIR}/stt.log" 2>&1 &
STT_PID=$!
...
echo -e "${GREEN}  AI Observer:    ${NC}http://localhost:$DARIA_PORT/static/debug_ai_observer.html"
echo -e "${GREEN}===========================================${NC}"
